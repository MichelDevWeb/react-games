...
import SOUND from "../assets/sounds/pixel_1.mp3";
...
class FullControl extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      loaded: false,
      loop: props.playing || true,
      mute: props.playing || false,
      volume: 0.0,
      seek: 0.0,
      rate: 1,
      isSeeking: false,
  ...
  handleChangeVolume(_volume) {
    if (_volume !== this.state.volume) {
      console.log(_volume);
      this.setState({ volume: parseFloat(_volume), playing: _volume > 0 });
